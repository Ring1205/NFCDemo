 ...
 * @date 2017/10/11
 */
public class MultifunctionalEditText extends AppCompatEditText {
    ...
    public MultifunctionalEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void setRightDrawable() {
        ...
        if (mDrawable == null) {
            switch (currentType) {
                case CLEAR:
                    mDrawable = getResources().getDrawable(R.mipmap.delete_text);
                    break;
        ...
        if (showIcon) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_UP:
                    Logger.i("mDrawable", getId() + " ________________event.getRawX() = " + event.getRawX() + " getRight() = " + getRight() + " mDrawable.getBounds().width() = " + mDrawable.getBounds().width());

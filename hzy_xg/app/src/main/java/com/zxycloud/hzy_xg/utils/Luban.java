...
public class Luban {
    ...
    private File thirdCompress(@NonNull File file) {
        String thumb = mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() + ".jpg" : filename);
        String copyPath = mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() + "1.jpg" : filename);
        ...
        Logger.e("Guke","filePath:"+filePath+"   thumb:"+thumb+"   copyPath:"+copyPath);
    ...
    private File compress(String largeImagePath, String thumbFilePath, int width, int height, int angle, long size) {
        ...
        FileUtils.delete(largeImagePath);
